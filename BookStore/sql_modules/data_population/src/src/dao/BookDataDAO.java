...
public class BookDataDAO {
	...
	public static void addBooks(List<Book> books, List<List<Author>> authors) throws SQLException {
		Connection con = ConnectionProvider.getConnection();
		try {
			con.setAutoCommit(false);
			for (int i = 0; i < books.size(); i++) {
				addBook(books.get(i), authors.get(i));
				 if (i % 10000 == 0) {
	            	System.out.println("book added : " + (i + 1));
	            }
			}
			con.commit();
		} catch (SQLException e) {
